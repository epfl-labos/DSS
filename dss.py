...
def verify_user_config(args):
    ...
    if args.duration_error is not None:
        if args.duration_error_mode is YarnErrorMode.CONSTANT and \
                args.duration_error_type is YarnErrorMode.MIXED:
    ...
    if args.mem_error is not None:
        if args.mem_error_mode is YarnErrorMode.CONSTANT and \
                args.mem_error_type is YarnErrorMode.MIXED:
...
def main():
    ...
    peek_args.add_argument("-p", "--peek-no-pushback", action="store_true",
                           help="Run a PEEK simulation for each potential ELASTIC container to avoid pushback.",
                           default=False)
    ...
    errinj_args.add_argument("--mem-error", type=int, help="Percentage by which to mis-estimate the ideal memory " +
                                                           "of the containers.")
    errinj_args.add_argument("--mem-error-type", type=get_enumparser(YarnErrorType), choices=YarnErrorType,
                             help="Whether memory error is positive, negative or either.",
                             default=YarnErrorType.MIXED.name)
    errinj_args.add_argument("--mem-error-mode", type=get_enumparser(YarnErrorMode), choices=YarnErrorMode,
                             help="Whether memory error is constant or random.", default=YarnErrorMode.CONSTANT.name)
