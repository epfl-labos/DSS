...
class YarnSimulationStartEvent(SimulationStartEvent):
    ...
    def handle(self):
        ...
        self.state.simulator.add_event(YarnOccupancyStatsEvent(self.state, False))
...
class YarnSimulationFinishEvent(SimulationFinishEvent):
    ...
    def handle(self):
        ...
        if not self.state.simulator.queue.empty():
            self.log.error(
...
class YarnContainerLaunchEvent(Event):
    ...
    def handle(self):
        ...
        if self.yarn_container.type is not YarnContainerType.MRAM:
            ...
            if self.state.user_config.duration_error is not None:
                ...
                if self.state.user_config.duration_error_mode is YarnErrorMode.CONSTANT:
                    ...
                    if self.state.user_config.duration_error_type is YarnErrorType.POSITIVE:
                        ...
                        upper_limit = 0
                    duration_adjustment = self.yarn_container.job.random.randint(lower_limit, upper_limit)
...
class YarnNodeContainerFinishEvent(Event):
    ...
    def handle(self):
        ...
        if self.yarn_container.type == YarnContainerType.MRAM:
            # Mark job as finished
            # NOTE: We do this here so that all events that check for job completion
            # can observe this.
            self.yarn_container.job.finished = True
